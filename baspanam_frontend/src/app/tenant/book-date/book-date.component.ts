...
export class BookDateComponent implements OnInit, OnDestroy {
    ...
    effect(() => {
      ...
      if (checkAvailabilityState.status === "OK") {
        ...
        this.toastService.send({
          severity: "error",
          detail: this.translate.instant('ERRORS.GET_DATES'),
  ...
  private onBookingSuccess(): void {
    this.toastService.send({
      severity: "success",
      detail: this.translate.instant('BOOKINGS.SUCCESS')
    });
  }

  private onBookingError(): void {
    this.toastService.send({
      severity: "error",
      detail: this.translate.instant('BOOKINGS.ERROR')
    });
  }
}
